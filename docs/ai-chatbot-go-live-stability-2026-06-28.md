...
- The queue runner now has a pre-worker fast path for clear casual turns, broad selected-hotel reservation-start inquiries, and deterministic selected-hotel booking signals such as room/date/guest-count messages. This prevents a first message like "I want to ask about Zad Ajyad", "how are you?", or "I want a double room" from waiting on a heavy worker before the guest sees a real assistant answer.
- Public support-case create/update also has a narrow immediate-reply guard for the same clear selected-hotel starts and room-selection turns. This protects the guest experience when admin PMS polling or reservation notification queries are slow enough to delay the full AI worker queue.
