...
Historically, the operator-facing list derived a HotelRunner-specific suffix from the canonical status plus trusted HotelRunner metadata. That longer label was never persisted in `state` or `reservation_status`. As of **2026-08-13**, the current list deliberately displays **OTA Platform Review** for every canonical review row, including rows with preserved HotelRunner provenance. Existing review filters, hotel-facing exclusions, permissions, reporting, pricing validation, and release concurrency continue to use the established canonical values.
...
- **2026-08-13:** Unified the `/admin/ota-reservations` Status-column presentation as **OTA Platform Review** for every canonical review row, including historical rows carrying HotelRunner metadata. This display-only change does not rewrite status, provenance, queue membership, filtering, pricing, or release behavior.
