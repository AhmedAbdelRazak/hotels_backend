...
The monitor implementation and systemd templates are versioned with the backend. They are intentionally not installed merely by deploying application code. Installation is a separate production operation after the release, index verification, and a clean manual audit.
...
- aggregate reason/provider and integrity-flag counts;
...
- `2`: active missing identity or incomplete authenticated pipeline delivery; state is `alert`

`alertFingerprint` hashes only the active OTA identity set and stable pipeline archive identifiers. Audit time, age buckets, query statistics, and terminal/expired findings do not change it. Transitions are `alert_new`, `alert_changed`, `alert_unchanged`, `resolved`, `clean_unchanged`, or `audit_error`.
